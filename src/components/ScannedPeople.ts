...
// Bound concurrent downloads/decodes and yield between geometries on touch devices.
async function eachLimited<T>(items:T[],limit:number,load:(item:T)=>Promise<void>){
 let next=0;
 await Promise.all(Array.from({length:limit},async()=>{
  while(next<items.length){const item=items[next++];await load(item);await new Promise(resolve=>setTimeout(resolve,0));}
 }));
}
...
export function scannedPeople(scene:T.Scene,renderer:T.WebGLRenderer,mobile=false){
 ...
 const ready=(async()=>{
  const response=await fetch(assetUrl("/people-v4/manifest.json"),{signal:AbortSignal.timeout(30000)});if(!response.ok)throw new Error('People manifest');const data=await response.json() as Manifest;
  const materialMap=new Map<string,T.MeshStandardMaterial>();
  await eachLimited(Object.entries(data.textures),mobile?2:4,async([name,url])=>{const texture=await new T.TextureLoader().loadAsync(assetUrl(mobile?url.replace(/([^/]+)$/,'mobile/$1'):url));if(!alive){texture.dispose();return;}texture.colorSpace=T.SRGBColorSpace;texture.flipY=name!=='headphones';texture.anisotropy=Math.min(4,renderer.capabilities.getMaxAnisotropy());textures.push(texture);const material=new T.MeshStandardMaterial({map:texture,roughness:.83,metalness:0,envMapIntensity:.45});mats.push(material);materialMap.set(name,material);});
  if(!alive)return;
  await eachLimited(data.records,mobile?2:4,async asset=>{
   const response=await fetch(assetUrl(asset.url),{signal:AbortSignal.timeout(30000)});if(!response.ok)throw new Error(asset.name);const encoded=new Uint8Array(await response.arrayBuffer());if(!alive)return;
   ...
   const mesh=new T.InstancedMesh(geo,material,placements.length),transform=new T.Object3D();mesh.name=asset.name;mesh.castShadow=!mobile;mesh.receiveShadow=!mobile;
